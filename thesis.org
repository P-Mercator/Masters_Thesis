...
#+PROPERTY: header-args:ipython :session kernel-bac61992-ed91-4fa8-8814-a8bfacdfbc31.json
...
** Comparison with Euclidean distance
   #+BEGIN_SRC ipython :exports none
     ...
     linkage_euc = hcl.ward(squareform(DM_euc))
     ...
     unique_labels = np.unique(labels)
     hcl.dendrogram(linkage_euc, labels = labels, color_threshold=300)
     my_palette = plt.cm.get_cmap("RdGy", len(unique_labels))
     label_color = {l:my_palette(i) for l, i in zip(unique_labels, np.arange(len(unique_labels)))}
     ax = plt.gca()
     xlbls = ax.get_xmajorticklabels()
     ...
     hcl.dendrogram(linkage_euc,
                 labels = labels, color_threshold=300)
     my_palette = plt.cm.get_cmap("RdGy", len(unique_labels))
     label_color = {l:my_palette(i) for l, i in zip(unique_labels, np.arange(len(unique_labels)))}
     ax = plt.gca()
     xlbls = ax.get_xmajorticklabels()
     ...
     consommation.index = pd.to_datetime(consommation.index, format="%Y-%m-%d")
     consommation = consommation.apply(zscore, axis=0)
     clusters = hcl.fcluster(linkage_euc, t=3, criterion="maxclust")
     #consommation = consommation.diff(365).iloc[365:,:]
     consommation.groupby(clusters, axis=1).mean().rolling(365,center=True).mean().plot(figsize=(10,5))
     plt.show()
     ...
     sil = [silhouette_score(DM_euc,
                             labels=hcl.fcluster(linkage_euc, t=n+2, criterion="maxclust"),
                             metric='precomputed')
            for n in range(100-2)]
     plt.figure()
     plt.plot(np.arange(100-2), np.array(sil))
     plt.show()
   #+END_SRC
    The RMSD was not necessary as the series all have the same length.
...
#+BEGIN_SRC R :exports none
  ...
  consommation <- diff(diff(as.matrix(consommation),7),1)
  consommation <- scale(consommation)
  res.dist <- dist(t(consommation), method='euclidean')
  as.matrix(res.dist)[1:3,1:3]
  res.hc <- hclust(d = res.dist, method = "average")
  fviz_nbclust(t(consommation), hcut, diss = res.dist, method = "silhouette", K.max=40)
  fviz_nbclust(t(consommation), hcut, diss = res.dist, method = "wss", K.max=40)
  fviz_nbclust(t(consommation), hcut, diss = res.dist, method = "gap_stat", K.max=40)

                                          # Elbow method
  fviz_nbclust(t(consommation), hcut, diss = res.dist, method = "wss",k.max=100)+
    ## geom_vline(xintercept = 2, linetype = 2)+
    labs(subtitle = "Elbow method")
                                          # Silhouette method
  fviz_nbclust(t(consommation), hcut, diss = res.dist, method = "silhouette",k.max=100)+
  ...
  set.seed(123)
  fviz_nbclust(t(consommation), hcut, diss = res.dist, nstart = 25,  method = "gap_stat", nboot = 3, k.max=100)+
    labs(subtitle = "Gap statistic method")

  fviz_dend(res.hc, rect = TRUE, color_labels_by_k = TRUE, # color labels by groups
            k = 3)
  ...
  res.hc2 <- hclust(res.dist, method = "ward.D")
  ...
  library(cluster)
  gs.pam.RU <- clusGap(t(consommation), agnes, 10, B = 100, d.power = 1,
        verbose = interactive())
#+END_SRC
